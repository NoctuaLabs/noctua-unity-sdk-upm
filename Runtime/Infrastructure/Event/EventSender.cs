...
namespace com.noctuagames.sdk.Events
{
        ...
        public void Send(string name, Dictionary<string, IConvertible> data = null)
        {
           ...
           if (TrackerObserverRegistry.HasObservers)
           {
            ...
            UniTask.Void(async () =>
            {
                ...
                if (_isSandbox != null) data.TryAdd("is_sandbox", _isSandbox);
	...
	// This will be called in SessionTracker.cs
        ...
        private string SanitizeHeaderValue(string value)
        {
            ...
            var filtered = System.Array.FindAll(value.ToCharArray(), c => c >= (char)32 && c != (char)127);
            var sanitized = new string(filtered);
            if (sanitized != value)
                _log.Info($"[Event Sender] Header value sanitized.");
            return sanitized;
        }
