...
public class LoadEventsTests
{
    ...
    // Ignored: Test with 1 billion events causes OutOfMemoryException
    // This exceeds available memory in the test environment (requires ~1GB+ just for JSON string)
    [TestCase(1_000_000_000)]
    [Ignore("Causes OutOfMemoryException - 1 billion events exceeds available test memory")]
    public void Test_LoadEvents_Performance_Extreme(int eventCount)
    {
        Debug.Log($"--- Testing LoadEventsFromPlayerPrefs with {eventCount:N0} events ---");
