...
- Add **Sentry-style `client_error` event forwarding** — `GlobalExceptionLogger` now forwards Unity `LogType.Warning`, `LogType.Error`, and `LogType.Exception` (plus `AppDomain.UnhandledException`) to the Noctua event pipeline via `EventSender.Send(...)` as a single `client_error` event. Payload carries `severity` (`warning`/`error`/`exception`), `error_type`, truncated `message` + `stack_trace`, `thread`, `scene`, `platform`, `app_version`, plus `dedup_count` and `suppressed_count` for storm analysis. Routed through `EventSender` directly (not `TrackCustomEvent`) so errors are **not** forwarded to Firebase/Adjust/Facebook native trackers. Rate-limited to 30 events/minute with 60s dedup and a `[ThreadStatic]` re-entrancy guard — safe for hot exception loops. Runtime kill-switch via `SetEventSender(null)`.
